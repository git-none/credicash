            ...
            while (isActive && !stopping) {
                ...
                try {
                    ...
                    if (lastWarnAtMs == 0L || now - lastWarnAtMs >= 30_000L) {
                        logger.warn(
                            "Credicash: canal de tiempo real temporalmente no disponible; nuevo intento en {} s. Causa: {}",
                            waitMs / 1_000L,
                            safeErrorMessage(error)
                        )
                        lastWarnAtMs = now
                    } else {
                        logger.debug("Canal de tiempo real aún no disponible; reintento en {} ms: {}", waitMs, safeErrorMessage(error))
                    }
                    delay(waitMs)
    ...
    fun stop() {
        ...
        activeConnection.getAndSet(null)?.let { connection ->
            runCatching { connection.close() }
                .onFailure { error -> logger.debug("No fue posible cerrar el canal PostgreSQL ya detenido: {}", error.message) }
        }
        subscribers.toList().forEach(::unsubscribe)
        logger.info("Credicash: canal PostgreSQL de tiempo real detenido limpiamente.")
    }
