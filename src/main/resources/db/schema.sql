...
-- Cambios 87: recuperación de contraseña exclusivamente mediante bot de Telegram.
CREATE TABLE IF NOT EXISTS solicitudes_recuperacion_telegram (
    id UUID PRIMARY KEY DEFAULT gen_random_uuid(),
    user_id BIGINT NOT NULL REFERENCES usuarios(id) ON DELETE CASCADE,
...
CREATE INDEX IF NOT EXISTS idx_telegram_password_reset_user
ON solicitudes_recuperacion_telegram(user_id, created_at DESC);
CREATE INDEX IF NOT EXISTS idx_telegram_password_reset_active
ON solicitudes_recuperacion_telegram(start_token_hash, request_expires_at)
WHERE consumed_at IS NULL;

INSERT INTO versiones_esquema(version, description)
VALUES (87, 'Recuperación de contraseña de uso exclusivo mediante Telegram')
ON CONFLICT (version) DO NOTHING;
...
-- Cambios 90: recuperación repetible por Telegram y restablecimiento conjunto de contraseña + PIN.
-- El chat_id verificado se conserva para que solicitudes posteriores puedan recibir un código nuevo
-- sin depender de que Telegram vuelva a emitir el payload /start del deep-link.
...
-- Recupera automáticamente vinculaciones válidas de versiones anteriores: el chat ya
-- tuvo que haber abierto un deep-link válido y recibido al menos un código del bot.
