...
import com.impulsosocial.server.integrations.TelegramBotService
...
private data class TelegramResetOutcome(
    val success: Boolean,
    val chatId: Long = 0L,
    val errorMessage: String? = null
)

private data class TelegramResetRequestOutcome(
    val userId: Long,
    val directChatId: Long = 0L,
    val directCode: String? = null,
    val recentlyDispatched: Boolean = false
)
...
class AppService(
    ...
    fun requestTelegramPasswordReset(request: TelegramPasswordResetRequest): TelegramPasswordResetResponse {
        verifyRecaptcha(request.recaptchaToken ?: request.captchaToken, "password_reset_request")
        if (!telegramBot.configured) {
            throw AppException("La recuperación por Telegram no está configurada en este momento.")
        }
        val identifier = request.identifier.trim()
        if (identifier.isBlank()) throw AppException("Ingresa tu usuario o correo de Kredi+.")
        val suppliedTelegramUsername = request.telegramUsername.orEmpty().trim().removePrefix("@").lowercase()
        if (suppliedTelegramUsername.isNotBlank() && !Regex("^[a-zA-Z0-9_]{5,32}$").matches(suppliedTelegramUsername)) {
            throw AppException("Ingresa un @ de Telegram válido.")
        }

        val tokenBytes = ByteArray(32).also(SecureRandom()::nextBytes)
        val startToken = Base64.getUrlEncoder().withoutPadding().encodeToString(tokenBytes)
        val tokenHash = sha256Hex(startToken)
        val requestOutcome = database.transaction { connection ->
            val user = connection.prepareStatement(
                "SELECT id FROM usuarios WHERE LOWER(username)=LOWER(?) OR LOWER(email)=LOWER(?) LIMIT 1"
            ).use { statement ->
                statement.setString(1, identifier)
                statement.setString(2, identifier)
                statement.executeQuery().use { result ->
                    if (!result.next()) throw AppException("No encontramos una cuenta Kredi+ con ese usuario o correo.")
                    result.getLong("id")
                }
            }

            val verifiedBinding = connection.prepareStatement(
                "SELECT telegram_chat_id,telegram_username FROM vinculaciones_recuperacion_telegram WHERE user_id=? LIMIT 1"
            ).use { statement ->
                statement.setLong(1, user)
                statement.executeQuery().use { rs ->
                    if (rs.next()) rs.getLong("telegram_chat_id") to rs.getString("telegram_username") else null
                }
            }

            val telegramUsername = when {
                verifiedBinding != null && suppliedTelegramUsername.isBlank() -> verifiedBinding.second
                suppliedTelegramUsername.isNotBlank() -> suppliedTelegramUsername
                else -> throw AppException("Ingresa tu @ de Telegram para vincular la recuperación por primera vez.")
            }
            if (verifiedBinding != null && !verifiedBinding.second.equals(telegramUsername, ignoreCase = true)) {
                val masked = verifiedBinding.second.let { value ->
                    when {
                        value.length <= 2 -> "**"
                        value.length <= 5 -> value.take(1) + "***"
                        else -> value.take(2) + "***" + value.takeLast(2)
                    }
                }
                throw AppException("Esta cuenta ya tiene un Telegram de recuperación vinculado (@$masked). Déjalo vacío o usa ese Telegram. El cambio de Telegram debe hacerse desde una sesión autenticada.")
            }

            // Evita que un doble toque o un reintento causado por una respuesta HTTP tardía
            // genere dos códigos distintos. Si el Telegram ya está verificado y existe un
            // código vigente enviado hace pocos segundos, se conserva ese mismo desafío.
            if (verifiedBinding != null) {
                val recentDispatch = connection.prepareStatement(
                    """
                    SELECT 1
                    ...
                    statement.setLong(1, user)
                    statement.setLong(2, verifiedBinding.first)
                ...
                if (recentDispatch) {
                    audit(
                        connection,
                        user,
                        "PASSWORD_RESET_TELEGRAM_REQUEST_DEDUPLICATED",
                        "USER",
                        user.toString(),
                        "Se reutilizó el código vigente para evitar una solicitud duplicada"
                    )
                    return@transaction TelegramResetRequestOutcome(
                        userId = user,
                        directChatId = verifiedBinding.first,
            ...
            connection.prepareStatement(
                "UPDATE solicitudes_recuperacion_telegram SET consumed_at=COALESCE(consumed_at,NOW()) WHERE user_id=? AND consumed_at IS NULL"
            ).use { statement ->
                statement.setLong(1, user)
                statement.executeUpdate()
            }
            val verifiedChatId = verifiedBinding?.first ?: 0L

            val directCode = if (verifiedChatId > 0L) {
                buildString(config.passwordResetCodeLength) {
                    val random = SecureRandom()
                    repeat(config.passwordResetCodeLength) { append(random.nextInt(10)) }
                }
            } else null

            if (directCode != null && verifiedChatId > 0L) {
                connection.prepareStatement(
                    """
                    INSERT INTO solicitudes_recuperacion_telegram(
                        user_id,telegram_username,start_token_hash,request_expires_at,
                        telegram_chat_id,code_hash,attempts,code_sent_at,code_expires_at
                    ) VALUES (?,?,?,NOW() + (? * INTERVAL '1 minute'),?,?,0,NOW(),NOW() + (? * INTERVAL '1 minute'))
                    """.trimIndent()
                ).use { statement ->
                    statement.setLong(1, user)
                    statement.setString(2, telegramUsername)
                    statement.setString(3, tokenHash)
                    statement.setLong(4, config.passwordResetRequestTtlMinutes)
                    statement.setLong(5, verifiedChatId)
                    statement.setString(6, passwordSecurity.hash(directCode))
                    statement.setLong(7, config.passwordResetCodeTtlMinutes)
                    statement.executeUpdate()
                }
            } else {
                connection.prepareStatement(
            ...
            audit(
                connection,
                user,
                "PASSWORD_RESET_TELEGRAM_REQUESTED",
                "USER",
                user.toString(),
                if (directCode != null) "Recuperación solicitada; código preparado para Telegram previamente verificado" else "Recuperación solicitada mediante enlace seguro de Telegram"
            )
            TelegramResetRequestOutcome(user, verifiedChatId, directCode)
        }

        if (requestOutcome.recentlyDispatched) {
            return TelegramPasswordResetResponse(
                message = "Ya te enviamos un código a Telegram. Usa el código más reciente para continuar con el restablecimiento.",
                telegramUrl = null,
                expiresInMinutes = config.passwordResetCodeTtlMinutes,
                status = "CODE_ALREADY_SENT",
                nextStep = "RESET_ACCESS"
            )
        }

        val deepLink = telegramBot.botDeepLink("reset_$startToken")
        if (requestOutcome.directCode != null && requestOutcome.directChatId > 0L) {
            val sent = runCatching {
                telegramBot.sendMessage(
                    requestOutcome.directChatId,
                    "🔐 <b>Nuevo código de recuperación Kredi+</b>\n\nTu código es: <b>${requestOutcome.directCode}</b>\n\nVence en ${config.passwordResetCodeTtlMinutes} minutos. No lo compartas con nadie."
                )
            }.getOrDefault(false)
            if (sent) {
                return TelegramPasswordResetResponse(
                    message = "Te enviamos un código nuevo al Telegram que ya verificaste. Puedes solicitar otro código cada vez que lo necesites.",
                    telegramUrl = null,
                    expiresInMinutes = config.passwordResetCodeTtlMinutes,
                    status = "CODE_SENT",
                    nextStep = "RESET_ACCESS"
        ...
        return TelegramPasswordResetResponse(
            message = "Abre Telegram. Si el bot ya estaba iniciado, usa el enlace o escribe /recuperar para recibir un código nuevo.",
            telegramUrl = deepLink,
            expiresInMinutes = config.passwordResetRequestTtlMinutes,
            status = "TELEGRAM_ACTION_REQUIRED",
            nextStep = "RESET_ACCESS"
        )
    }

    fun handleTelegramPasswordRecovery(update: TelegramUpdate) {
        val message = update.message ?: return
        val text = message.text.orEmpty().trim()
        val payloadMatch = Regex("^/start(?:@[A-Za-z0-9_]+)?\\s+reset_([A-Za-z0-9_-]{20,})$").find(text)
        val plainRecoveryCommand = Regex("^/(?:start|recuperar|codigo)(?:@[A-Za-z0-9_]+)?$", RegexOption.IGNORE_CASE).matches(text)
        if (payloadMatch == null && !plainRecoveryCommand) return

        val chatId = message.chat?.id ?: return
        val telegramUsername = message.from?.username?.trim()?.lowercase()
        if (telegramUsername.isNullOrBlank()) {
            telegramBot.sendMessage(chatId, "Para recuperar tus datos de acceso necesitas tener un @usuario configurado en Telegram.")
            return
        }
        val tokenHash = payloadMatch?.groupValues?.get(1)?.let(::sha256Hex)

        val result = runCatching {
            database.transaction { connection ->
                val row = if (tokenHash != null) {
                    connection.prepareStatement(
                        """
                        SELECT id,user_id,telegram_username
                        FROM solicitudes_recuperacion_telegram
                        WHERE start_token_hash=? AND consumed_at IS NULL AND request_expires_at>NOW()
                        FOR UPDATE
                        """.trimIndent()
                    ).use { statement ->
                        statement.setString(1, tokenHash)
                        statement.executeQuery().use { rs ->
                            if (!rs.next()) throw AppException("Este enlace de recuperación venció o ya fue utilizado. Solicita uno nuevo desde Kredi+.")
                            Triple(rs.getObject("id", UUID::class.java), rs.getLong("user_id"), rs.getString("telegram_username"))
                        }
                    }
                } else {
                    connection.prepareStatement(
                        ...
                        SELECT id,user_id,telegram_username
                        FROM solicitudes_recuperacion_telegram
                        WHERE LOWER(telegram_username)=LOWER(?) AND consumed_at IS NULL AND request_expires_at>NOW()
                        ORDER BY created_at DESC
                        LIMIT 1 FOR UPDATE
                        """.trimIndent()
                    ).use { statement ->
                        ...
                        statement.executeQuery().use { rs ->
                            if (!rs.next()) throw AppException("No hay una recuperación activa para @$telegramUsername. Solicita una nueva desde Kredi+ y vuelve a escribir /recuperar.")
                            Triple(rs.getObject("id", UUID::class.java), rs.getLong("user_id"), rs.getString("telegram_username"))
                        }
                    }
                }

                if (!row.third.equals(telegramUsername, ignoreCase = true)) {
                    throw AppException("Esta recuperación fue solicitada para @${row.third}. Abre Telegram con ese usuario o inicia una nueva recuperación.")
                }

                val code = buildString(config.passwordResetCodeLength) {
                    val random = SecureRandom()
                    repeat(config.passwordResetCodeLength) { append(random.nextInt(10)) }
                }
                ...
                connection.prepareStatement(
                    """
                    UPDATE solicitudes_recuperacion_telegram
                    SET telegram_chat_id=?, code_hash=?, attempts=0, code_sent_at=NOW(),
                        code_expires_at=NOW() + (? * INTERVAL '1 minute')
                    WHERE id=?
                    """.trimIndent()
                ).use { statement ->
                    statement.setLong(1, chatId)
                    statement.setString(2, codeHash)
                    statement.setLong(3, config.passwordResetCodeTtlMinutes)
                    statement.setObject(4, row.first)
                    statement.executeUpdate()
                }
                audit(connection, row.second, "PASSWORD_RESET_TELEGRAM_CODE_SENT", "USER", row.second.toString(), "Código temporal nuevo enviado al chat de Telegram solicitado")
                code
            }
        }
        ...
        result.onSuccess { code ->
            telegramBot.sendMessage(
                chatId,
                "🔐 <b>Código de recuperación Kredi+</b>\n\nTu código es: <b>$code</b>\n\nVence en ${config.passwordResetCodeTtlMinutes} minutos. Si solicitas otro, este código dejará de ser válido."
            )
        }.onFailure { error ->
            telegramBot.sendMessage(chatId, error.message ?: "No fue posible procesar esta recuperación. Solicita una nueva desde Kredi+.")
        }
    }

    fun resetPasswordWithTelegram(request: TelegramPasswordResetConfirmRequest): MessageResponse {
        ...
        if (identifier.isBlank()) throw AppException("Primero solicita una recuperación desde Kredi+.")
        if (code.length != config.passwordResetCodeLength) throw AppException("Ingresa los ${config.passwordResetCodeLength} dígitos del código.")

        val resetResult = database.transaction { connection ->
            database.ensureAuthenticationSchema(connection)
            val userInfo = connection.prepareStatement(
                "SELECT id,username,email FROM usuarios WHERE LOWER(username)=LOWER(?) OR LOWER(email)=LOWER(?) LIMIT 1 FOR UPDATE"
            ).use { statement ->
                statement.setString(1, identifier)
                statement.setString(2, identifier)
                statement.executeQuery().use { rs ->
                    if (!rs.next()) throw AppException("La solicitud de recuperación no es válida.")
                    Triple(rs.getLong("id"), rs.getString("username"), rs.getString("email"))
                }
            }
            PasswordPolicy.validationError(newPassword, userInfo.second, userInfo.third)?.let { throw AppException(it) }
            PinPolicy.validationError(newPin)?.let { throw AppException(it) }

            val recovery = connection.prepareStatement(
                """
                SELECT id,code_hash,attempts,telegram_chat_id,telegram_username
                FROM solicitudes_recuperacion_telegram
                WHERE user_id=? AND consumed_at IS NULL AND code_hash IS NOT NULL
                  AND code_expires_at>NOW() AND attempts<?
                ORDER BY created_at DESC
                LIMIT 1 FOR UPDATE
                """.trimIndent()
            ).use { statement ->
                statement.setLong(1, userInfo.first)
                statement.setInt(2, config.passwordResetMaxAttempts)
                statement.executeQuery().use { rs ->
                    if (!rs.next()) return@transaction TelegramResetOutcome(
                        success = false,
                        errorMessage = "El código venció o alcanzó el máximo de intentos. Solicita uno nuevo."
                    )
                    listOf(
                        rs.getObject("id", UUID::class.java), rs.getString("code_hash"), rs.getInt("attempts"),
                        rs.getLong("telegram_chat_id"), rs.getString("telegram_username")
                    )
                }
            }
            val recoveryId = recovery[0] as UUID
            val codeHash = recovery[1] as String
            val attempts = recovery[2] as Int
            val chatId = recovery[3] as Long
            val telegramUsername = recovery[4] as String
            connection.prepareStatement("UPDATE solicitudes_recuperacion_telegram SET attempts=attempts+1 WHERE id=?").use { statement ->
                statement.setObject(1, recoveryId)
                statement.executeUpdate()
            }
            if (!passwordSecurity.verify(codeHash, code)) {
                if (attempts + 1 >= config.passwordResetMaxAttempts) {
                    connection.prepareStatement("UPDATE solicitudes_recuperacion_telegram SET consumed_at=NOW() WHERE id=?").use { statement ->
                        statement.setObject(1, recoveryId)
                        statement.executeUpdate()
                    }
                }
                return@transaction TelegramResetOutcome(
                    success = false,
                    chatId = chatId,
                    errorMessage = "El código no es correcto. Verifica los 6 dígitos e inténtalo nuevamente."
                )
            }

            connection.prepareStatement(
                "UPDATE usuarios SET password_hash=?, pin_hash=?, failed_login_attempts=0, locked_until=NULL, updated_at=NOW() WHERE id=?"
            ).use { statement ->
                statement.setString(1, passwordSecurity.hash(newPassword))
                statement.setString(2, passwordSecurity.hash(newPin))
                statement.setLong(3, userInfo.first)
                statement.executeUpdate()
            }
            connection.prepareStatement(
                "UPDATE sesiones_usuario SET revoked_at=COALESCE(revoked_at,NOW()), ended_reason=COALESCE(ended_reason,'ACCESS_RESET_TELEGRAM') WHERE user_id=? AND revoked_at IS NULL"
            ).use { statement ->
                statement.setLong(1, userInfo.first)
                statement.executeUpdate()
            }
            connection.prepareStatement(
                "UPDATE credenciales_biometricas_dispositivo SET enabled=FALSE, updated_at=NOW() WHERE user_id=? AND enabled=TRUE"
            ).use { statement ->
                statement.setLong(1, userInfo.first)
                statement.executeUpdate()
            }
            connection.prepareStatement(
                "UPDATE desafios_autenticacion SET used_at=COALESCE(used_at,NOW()) WHERE user_id=? AND purpose='LOGIN_PIN' AND used_at IS NULL"
            ).use { statement ->
                statement.setLong(1, userInfo.first)
                statement.executeUpdate()
            }
            ...
            audit(connection, userInfo.first, "ACCESS_RESET_TELEGRAM_COMPLETED", "USER", userInfo.first.toString(), "Contraseña y PIN actualizados mediante Telegram; sesiones, PIN-challenges y biometría anteriores invalidados")
            TelegramResetOutcome(success = true, chatId = chatId)
        }

        if (!resetResult.success) throw AppException(resetResult.errorMessage ?: "No fue posible actualizar tus datos de acceso.")
        if (resetResult.chatId > 0L) {
            runCatching { telegramBot.sendMessage(resetResult.chatId, "✅ Tu contraseña y PIN de Kredi+ fueron actualizados correctamente. Las sesiones y accesos biométricos anteriores se cerraron por seguridad.") }
        }
        return MessageResponse("Contraseña y PIN actualizados. Inicia sesión nuevamente y vuelve a activar la biometría si deseas usarla.")
    }
    ...
    fun updateCombo(adminId: Long, comboId: Long, request: CreateComboRequest): ComboDto = database.transaction { connection ->
        ...
        val currentCoverPath = connection.prepareStatement(
            "SELECT cover_path FROM combos WHERE id=? AND active=TRUE FOR UPDATE"
        ).use { statement ->
        ...
        connection.prepareStatement("UPDATE combos SET name=?,description=? WHERE id=? AND active=TRUE").use { statement ->
            statement.setString(1, cleanName)
            statement.setString(2, cleanDescription)
            statement.setLong(3, comboId)
        ...
        audit(connection, adminId, "ADMIN_UPDATED_COMBO", "COMBO", comboId.toString(), "Combo actualizado: $cleanName")
        ComboDto(
            id = comboId,
            name = cleanName,
            ...
            active = true,
            coverUrl = publicUrl(currentCoverPath)
        )
    }
    ...
    fun combos(): List<ComboDto> = database.dataSource.connection.use { connection ->
        connection.prepareStatement("SELECT id,name,description,active,cover_path FROM combos WHERE active=TRUE ORDER BY created_at DESC").use { statement ->
            statement.executeQuery().use { result -> buildList {
                while (result.next()) {
                    val id = result.getLong(1)
                    add(ComboDto(
            ...
            WITH public_ledger AS (
                SELECT
                    ...
                    CASE t.operation_type
                        ...
                        WHEN 'ADMIN_TO_USER_TRANSFER' THEN 'Transferencia de saldo Crédito Kredi+ a una cartera de usuario'
                ...
                SELECT
                    ...
                    'Aprobación y desembolso de crédito Crédito Kredi+'::TEXT AS description,
                ...
                SELECT
                    ...
                    CASE ct.transaction_type
                        WHEN 'PURCHASE' THEN 'Compra financiada mediante Crédito Kredi+'
                        WHEN 'INSTALLMENT_PAYMENT' THEN 'Pago de cuota Crédito Kredi+'
                        WHEN 'REFUND' THEN 'Reintegro Crédito Kredi+'
                        WHEN 'ADJUSTMENT' THEN 'Ajuste contable Crédito Kredi+'
                        ELSE 'Movimiento Crédito Kredi+'
                    END::TEXT AS description,
    ...
    fun createPurchase(userId: Long, request: CreatePurchaseRequest): PurchaseDto {
        val creditRate = if (request.paymentMethod == "CREDIMPULSO") {
            runCatching { bcvRateService.currentUsdRate().rate }
                .getOrElse { throw AppException("No se pudo consultar la tasa BCV para calcular Crédito Kredi+. Intenta nuevamente.") }
        ...
        val purchase = database.transaction { connection ->
            ...
            val appliedPromotion = bestPromotionForPurchase(connection, request.fairId, directTotalsByProduct, comboTotalsByCombo, subtotalBd, promotionRate)
            ...
            PurchaseDto(orderId, total, itemCount, orderStatus, invoiceNumber, System.currentTimeMillis(), request.paymentMethod, request.fairId, paymentDestinationDto(connection, request.fairId), subtotalBd.toDouble(), discountBd.toDouble(), appliedPromotion?.name)
        }
    ...
    fun deleteUserPermanently(actorId: Long, targetUserId: Long) {
        ...
        try {
            database.transaction { connection ->
                ...
                val protectedReferences = protectedUserDeletionReferences(connection, targetUserId)
                if (protectedReferences.isNotEmpty()) {
                    logger.info(
                        "Eliminación física rechazada para userId={}: historial protegido en {}",
                        targetUserId,
                        protectedReferences.joinToString(",")
                    )
                    throw AppException(
                        "No se puede eliminar definitivamente esta cuenta porque posee actividad histórica que debe conservarse. " +
                            "Si necesitas impedir el acceso, utiliza la suspensión o el retiro de permisos según corresponda."
                    )
                }

                // Una cuenta verificada puede tener filas de cartera creadas automáticamente aun sin
                // compras, pagos ni créditos. Esas filas son estado recreable y no deben impedir una
                // eliminación definitiva de una cuenta realmente vacía.
                deleteEphemeralUserState(connection, targetUserId)
            ...
            val sqlError = generateSequence(error) { it.cause }
                ...
                .firstOrNull()
            val protectedByDatabase = sqlError?.sqlState == "23503" ||
                (sqlError?.sqlState == "P0001" && sqlError.message.orEmpty().contains("inmutable", ignoreCase = true))
            if (protectedByDatabase) {
                throw AppException(
                    "No se puede eliminar definitivamente esta cuenta porque todavía posee registros históricos protegidos que deben conservarse. " +
                        "Si necesitas impedir el acceso, utiliza la suspensión o el retiro de permisos según corresponda."
                )
            }
            throw error
    ...
    fun importProductsExcel(actorId: Long, bytes: ByteArray): ProductExcelImportResultDto {
        ...
        if (createdIds.isNotEmpty()) {
            notifyBeneficiaries("Nuevos productos", "Se incorporaron ${createdIds.size} producto(s) al catálogo de Kredi+.", "NEW_PRODUCTS_IMPORT", mapOf("count" to createdIds.size.toString()))
        }
    ...
    private fun permissionsFor(role: String, subRole: String): List<String> = when (role) {
        ...
        Roles.ACCOUNTANT -> listOf(
            ...
            "VIEW_USERS", "REVIEW_USERS", "MANAGE_STAFF_ROLES"
        )
    ...
    private fun fairDto(connection: Connection, fairId: Long): FairDto {
        ...
        val offers = connection.prepareStatement(
            """SELECT fp.product_id,p.base_price,fp.image_path,p.bcv_rate
            ...
            statement.setLong(1, fairId); statement.executeQuery().use { result -> buildList {
                while (result.next()) {
                    ...
                    add(FairProductOfferDto(productId, finalPrice.toDouble(), publicUrl(result.getString(3)), original.toDouble(), discount.toDouble(), best?.first?.name, label))
                }
            } }
        }
