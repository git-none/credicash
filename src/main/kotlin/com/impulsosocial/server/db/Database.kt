...
class Database(private val config: AppConfig) {
    ...
    fun verifyRequiredSchema() {
        val requiredTables = listOf(
            ...
            "solicitudes_recuperacion_telegram",
            "vinculaciones_recuperacion_telegram",
        ...
        dataSource.connection.use { connection ->
            ...
            connection.prepareStatement(
                "SELECT EXISTS(SELECT 1 FROM versiones_esquema WHERE version = 87)"
            ).use { statement ->
                statement.executeQuery().use { result ->
                    check(result.next() && result.getBoolean(1)) {
                        "La migración 87 de recuperación por Telegram no está registrada en versiones_esquema."
                    }
                }
            }
            ...
            connection.prepareStatement(
                "SELECT EXISTS(SELECT 1 FROM versiones_esquema WHERE version = 90)"
            ).use { statement ->
                statement.executeQuery().use { result ->
                    check(result.next() && result.getBoolean(1)) {
                        "La migración 90 de recuperación repetible por Telegram no está registrada en versiones_esquema."
                    }
                }
            }
